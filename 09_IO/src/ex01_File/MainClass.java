...
public class MainClass {
	...
	*/
	public static void ex01() {
		...
		File dir = new File("C:" + File.separator + "storage");				// C드라이브 아래 storage 폴더를 의미 역슬래시를 넣으려면 앞으로 필수로 두개씩 넣어야함
	...
	public static void ex03() throws IOException {
		...
		System.out.println("디렉터리인가?" + file.isDirectory());	//	디렉터리라면 트루
		System.out.println("파일인가?" + file.isFile());		//	파일이라면 트루
		
		long lastModified = file.lastModified();	//	최종 수정일의 타임스탬프 값이 나온
		...
		long size = file.length();	//	파일의 크기가 바이트 단위로 계산
	...
	public static void main(String[] args) throws IOException {	//	main 메소드를 호출하는 곳으로 예외 처리를 한다.(개발자가 try-catch 하지 않겠다)
		ex02();
	}
