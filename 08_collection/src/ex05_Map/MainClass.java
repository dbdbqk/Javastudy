...
public class MainClass {
	...
	public static void ex01() {
		...
		/* 
			...
			2. 용어 정리
				1) Entry : 하나의 데이터
				2) Key	 : Entry의 구성 요소, 식별자 역할을 수행(배열의 인덱스와 같은 역할)	*식별자 역할 (Key) *
				3) Value : Entry의 구성 요소, 실제 데이터(배열에 저장된 데이터와 같은 역할)
			3. 주로 Key는 String을 사용한다. (변수 이름) *변수이름 (Key)*
			4. 주로 Value는 Object를 사용한다.		
			5. Key는 중복이 불가능하고, Value는 중복이 가능하다.
			6. 맵은 객체를 대체 가능하다.
		*/
		
		// Person 정보를 HashMap으로 만들기     // 이걸 제일 많이 씀
		Map<String, Object> person = new HashMap<String, Object>();
		
		// 추가
		// put(Key, Value)
		person.put("name", "홍길동");	// 엔트리라 부름	
		person.put("age", 30);			// 엔트리라 부름
	...
	public static void ex03() {
		...
		// 2. Entry를 모두 가져온 뒤, Key와 Value로 분리하기     이 방법을 선호
	...
	public static void ex04() {
		...
		book2.put("title", "책제목-2");
		book2.put("author", "책저자-2");
		book2.put("price", 20000);
		
		Map<String, Object> book3 = new HashMap<String, Object>();
		book3.put("title", "책제목-3");
		book3.put("author", "책저자-3");
		...
		for(int i = 0, length = books.size(); i < length; i++ ) {
			...
			Map<String, Object> book = books.get(i); // List 에서는 books.get(i)다
